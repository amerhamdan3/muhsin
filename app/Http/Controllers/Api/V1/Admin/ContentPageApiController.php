...
class ContentPageApiController extends Controller
{
    ...
    public function store(StoreContentPageRequest $request)
    {
        ...
        if ($request->input('images', false)) {
            $contentPage->addMedia(storage_path('tmp/uploads/' . basename($request->input('images'))))->toMediaCollection('images');
        }
    ...
    public function update(UpdateContentPageRequest $request, ContentPage $contentPage)
    {
        ...
        if ($request->input('images', false)) {
            if (!$contentPage->images || $request->input('images') !== $contentPage->images->file_name) {
                if ($contentPage->images) {
                    $contentPage->images->delete();
                }
                $contentPage->addMedia(storage_path('tmp/uploads/' . basename($request->input('images'))))->toMediaCollection('images');
            }
        } elseif ($contentPage->images) {
            $contentPage->images->delete();
        }
