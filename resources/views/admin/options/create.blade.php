    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.options.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('blogs') ? 'is-invalid' : '' }}" name="blogs[]" id="blogs" multiple>
                    @foreach($blogs as $id => $blog)
                        <option value="{{ $id }}" {{ in_array($id, old('blogs', [])) ? 'selected' : '' }}>{{ $blog }}</option>
                    @endforeach
                </select>
                @if($errors->has('blogs'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('belongto') ? 'is-invalid' : '' }}" type="text" name="belongto" id="belongto" value="{{ old('belongto', '') }}">
